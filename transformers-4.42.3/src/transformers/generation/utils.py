...
class GenerationMixin:
    ...
    def dola_sample(
        ...
        **model_kwargs,
        ...
        # stopping_criteria=StoppingCriteriaList(MaxLengthCriteria(max_length=stopping_criteria.max_length))
        ...
        while True:
            ...
            if base_layer is not None:
                ...
                final_logits = dict_outputs[mature_layer][0, -1, :]
                # print(final_logits,final_logits.shape) 
                # ORG-finallogits shape： torch.Size([1, 32000])
                ...
                if relative_top > 0.0:
                    ...
                    # print(relative_top_mask,relative_top_mask.shape)
                    ...
                    # false_indices tensor([  382,   960, 14269], device='cuda:0') torch.Size([3])
                    
                    logits_values = final_logits[false_indices].view(-1).cpu().numpy()
                    
                    indices_values = false_indices.view(-1).cpu().numpy()
                    # indices_values [  382   960 14269] (3,)
            
                    sl_dict_outputs = torch.stack([dict_outputs[i][0, -1, :] for i in candidate_premature_layers],dim=0)
                    # print(sl_dict_outputs.shape)                
                    sl_dict_outputs = sl_dict_outputs[:,indices_values]
                    distribution = []
                    # distribution_tensor = torch.stack([dict_outputs[layer][0, -1, :] for layer in candidate_premature_layers], dim=0)
                    
                    distribution_tensor = sl_dict_outputs.permute(1, 0) # shape: (selected_voc, num_layers)
                    ...
                    entropy = entropy.unsqueeze(dim=0)
                    false_indices = false_indices.unsqueeze(dim=0)

                    final_logits[false_indices] = final_logits[false_indices]+ en_alpha*(-entropy)

                    # nt_entropy = dict(zip(false_indices.view(-1).cpu().numpy(),entropy.view(-1).cpu().numpy()))
                    # print(entropy)


#  visualize of vocablary-level cross layer logits
                    ...
                    # # js_list = js_divs.view(-1).cpu().numpy()*10000

                    # kl_list = F.kl_div(log_softmax_premature_layers, softmax_mature_layer, reduction='none').mean(-1)
                    # js_list = kl_list.view(-1).cpu().numpy()*10000


                    # if True:
                    # import os
                    #     # app = final_logits[false_indices].view(-1).cpu().numpy()
                    #     # print('After: ',app,'\n','='*10)
                    #     a_false_indices = false_indices.view(-1).cpu().numpy()
                    #     # print('='*10,'\n',a_false_indices,'\n')
                    #     a_entropy = entropy.view(-1).cpu().numpy()
                    #     # print('\n',a_entropy,'\n')
                    #     ap = final_logits[false_indices].view(-1).cpu().numpy()
                    #     # print('Before: ',ap,'\n')
    
                    #     file_path = "../entropy/Gentropy_7B_0.0001.txt"
                    ...
                    #         f.write("a = {")
                    #         f.write(f"'base' : np.array([")
                    #         for logits in ap:
                    #             f.write(f"{logits}")
                    #             if logits != ap[-1]:
                    #                 f.write(f",")
                    #         f.write(f"]),\n")
                    #         f.write(f"'entropy' : np.array([")
                    #         for value in a_entropy:
                    #             f.write(f"{value}")
                    #             if value != a_entropy[-1]:
                    #                 f.write(f",")
                    #         f.write(f"]),\n")
                    #         f.write(f"'indices' : [")
                    #         for index in a_false_indices:
                    #             f.write(f"{index}")
                    #             if index != a_false_indices[-1]:
                    #                 f.write(f",")
                    #         f.write(f"]")
                    #         f.write("}\nlist.append(a)\n")
                    #         # print(f"Entropy values written to {file_path}")


                    final_logits = final_logits.unsqueeze(dim=0)
                    next_token_logits = final_logits           
                else:
                    print('='*20,'no filter rate set','='*20)
                    # exit(0)
                    sl_dict_outputs = torch.stack([dict_outputs[i][0, -1, :] for i in candidate_premature_layers],dim=0)
                    ...
                    entropy = torch.distributions.Categorical(probs=probs, validate_args=False).entropy()
                    entropy = entropy.unsqueeze(dim=0)
                    final_logits = final_logits+ en_alpha*(-entropy)
                    # final_logits = final_logits.unsqueeze(dim=0)
                    next_token_logits = final_logits           
 
                    
          # next_token_logits torch.Size([1, 32000])
...
# -------------------------------------------------------------------------------------------
            ...
            # sp.load('/data/models/Llama-2-7b-chat-hf/tokenizer.model')
            # idx = next_tokens.item()
            # nt_logits = torch.stack([dict_outputs[i][0, -1, :] for i in candidate_premature_layers],dim=0)
            # # print(nt_logits.shape)
            # # nt_logits = nt_logits[:,idx].view(-1).cpu().numpy()
            # # # nt_logits = nt_logits[:-1,idx]
            # # nt_logits = nt_logits[:,:]
            # # # nt_logits = nt_logits[:-1,:]
            # nt_logits = nt_logits.permute(1, 0)
            
            # probs = torch.softmax(nt_logits,dim=1)
            # nt_entropy = torch.distributions.Categorical(probs=probs, validate_args=False).entropy()

            # entropy_list = []
            # for layer in range(1,len(candidate_premature_layers)):
            #     temp_logits = torch.stack([dict_outputs[i][0, -1, indices_values] for i in candidate_premature_layers[0:layer+1]],dim=0)
            #     # print(candidate_premature_layers[0:layer+1])
            #     temp_logits = temp_logits.permute(1, 0)
            #     temp_probs = torch.softmax(temp_logits,dim=1)
            #     temp_entropy = torch.distributions.Categorical(probs=temp_probs, validate_args=False).entropy().view(-1).cpu().numpy()
            #     # print(temp_entropy.view(-1).cpu().numpy())
            #     entropy_list.append(temp_entropy)
                

#             et = nt_entropy[idx].item()
#             word = sp.id_to_piece(idx)
 


#             # file_path = "../entropy/Theory_PO+1430.txt"
#             file_path = "../entropy/Theory_PO1630a.txt"
#             with open(file_path, 'a') as f:
                
#                 # print('\nIndex: ',idx, ' \tWord: ',word,'\tEntropy: ',et,'\nLogits: ',nt_logits,'\n')
#                 # f.write(f"\nIndex: {idx}\tWord {word}\tEntropy: {et}\n{nt_logits}\n")
#                 f.write(f"\nIndex: {idx}\tWord {word}\tEntropy: {et}\n\n")
#                 print('\nIndex: ',idx, ' \tWord: ',word,'\nLogits: ',nt_logits,'\n')
#                 # print(f'{idx},')
...
#                 #     i = int(i)
#                 #     i_word = sp.id_to_piece(i)
#                 #     i_et = nt_entropy[i].item()
#                 #     print('Index: ',i, ' \tWord: ',i_word,'\tEntropy: ',i_et)
#                 #     f.write(f"\nIndex: {i}\tWord: {i_word}\tEntropy: {i_et}")
#                 # f.write("\n")
...
#                     # i_word = sp.id_to_piece(i)
#                     f.write(f"{i}")
#                     if i != indices_values[-1]:
#                         f.write(f",")
#                 f.write(f"],\n")
                
#                 f.write(f"'logits' : np.array([")
#                 for i in indices_values:
#                     i = int(i)
#                     f.write(f"[")
#                     ap = nt_logits[i].view(-1).cpu().numpy()
#                     print('\nIndex: ',i,'\tWord:',word,'\nLogits: ',ap)
#                     for logits in ap:
#                         f.write(f"{logits}")
#                         if logits != ap[-1]:
...
#                 # f.write(f"'entropy' : np.array([")
#                 # for l in range(len(candidate_premature_layers)-1):
#                 #     entropy_temp = entropy_list[l]
#                 #     f.write(f"[")
#                 #     print('\nLayer: ',l,'\tEntropies:',entropy_temp)
#                 #     for entropy in entropy_temp:
#                 #         f.write(f"{entropy}")
#                 #         if entropy != entropy_temp[-1]:
#                 #             f.write(f",")
                            
#                 #     f.write(f"]")
#                 #     if l != len(candidate_premature_layers)-2:
#                 #         f.write(f",")
...
# # -------------------------------------------------------------------------------------------              
            ...
            # file_path = "../entropy/Theory_S1430.txt"
            # with open(file_path, 'a') as f:
            #     f.write(f"\n {word}\t{etp}\t{nt_logits}\n")
            #         f.write("a = {")
            #         f.write(f"'base' : np.array([")
